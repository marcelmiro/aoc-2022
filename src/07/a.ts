...
// Get total sizes of all dirs in `tree`
const dirSizes: number[] = []

function getDirsWithSize(tree: Tree) {
	for (const name of Object.keys(tree)) {
		const value = tree[name]
		if (isTree(value)) {
			dirSizes.push(value.totalSize)
			getDirsWithSize(value.children)
		}
	}
}

getDirsWithSize(tree)

// Sum total sizes of dirs with max size 100,000
const totalSizeOfDirs = dirSizes
	.filter((size) => size <= 100_000)
	.reduce((sum, cur) => sum + cur, 0)

console.log(totalSizeOfDirs)

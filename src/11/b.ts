...
// Prevent big worry levels by using remainder (must not conflict
// with monkey `operation` divisors and output incorrect worry levels)
const remainderDiv = monkeys.reduce(
	(total, monkey) => total * monkey.divisor,
	1
)
	...
	for (const [j, monkey] of monkeys.entries()) {
		...
		for (let k = 0; k < itemLength; k++) {
			...
			const worry = Math.floor(
				monkey.operation(monkey.items[0]) % remainderDiv
			)
